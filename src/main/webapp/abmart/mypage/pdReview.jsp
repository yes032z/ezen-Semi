...
<%
	...
	try{
		list=reviewService.selectReviewByid(id, startDate, lastDate);
		...
		<div id="orderinfo-third">
			<table class="table table-hover">
				...
				<%}else{
			  	...
			  	for(int i=0;i<pageVo.getPageSize();i++){
			  		...
			  		int reviewGrade = vo.getReviewgrade();
			  		num--;
			  	%>	
					<tr>
			...
			<div class="divPage">
				...
				<%if(pageVo.getFirstPage()>1){%>
					<a href="pdReview.jsp?currentPage=<%=pageVo.getFirstPage()-1%>">◀</a>	
				<%} %>
			   ...
			   <%   }else{   %>
			         <a href="pdReview.jsp?currentPage=<%=i%>&startDate=<%=startDate%>&lastDate=<%=lastDate %>">[<%=i %>]</a>
			   <%   }//if      
				}//for %>
				
				<!-- 다음 블럭으로 이동 -->
				<%if(pageVo.getLastPage()< pageVo.getTotalPage()){%>
					<a href="pdReview.jsp?currentPage=<%=pageVo.getLastPage()+1%>">▶</a>	
				<%} %>
